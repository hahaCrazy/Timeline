...
Shader "ZingFront/Toon_DoubleShadeWithFeather" {
    Properties {
...
//ToonCoreStart
        Pass {
            ...
            #include "ZingFrontLighting.cginc"
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float3 lightColor = saturate(_LightColor0.rgb);
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 lightColor = saturate(_LightColor0.rgb);
                float3 halfDirection = normalize(normalize(viewDirection)+normalize(lightDirection));
////// Lighting:
                ...
                float3 Set_LightColor = saturate(_LightColor0.rgb);
                ...
                // fixed4 finalRGBA = fixed4(DepressLighting(finalColor, node_9970.rgb),1);
...
////// Lighting:
                ...
                float2 node_6858 = Set_UV0;
                float4 _BaseMap_var = tex2D(_BaseMap,TRANSFORM_TEX(node_6858, _BaseMap));
                // mix of base color and base map color
                float3 node_9970 = (_BaseColor.rgb*_BaseMap_var.rgb);
                float3 Set_LightColor = saturate(_LightColor0.rgb);
                // _Is_lightColor_base = 1
                // base color * base map color * _LightColor0
                float3 Set_BaseColor = lerp( node_9970, (node_9970*Set_LightColor), _Is_LightColor_Base );

                float2 node_8098 = Set_UV0;
                float4 _1st_ShadeMap_var = tex2D(_1st_ShadeMap,TRANSFORM_TEX(node_8098, _1st_ShadeMap));
                float3 node_6918 = (_1st_ShadeColor.rgb*_1st_ShadeMap_var.rgb);
                // _1st_ShadeColor * _1st_ShadeMap_var * _LightColor0
                float3 Set_1st_ShadeColor = lerp( node_6918, (node_6918*Set_LightColor), _Is_LightColor_1st_Shade );

                float2 node_5380 = Set_UV0;
                float4 _2nd_ShadeMap_var = tex2D(_2nd_ShadeMap,TRANSFORM_TEX(node_5380, _2nd_ShadeMap));
                float3 node_8559 = (_2nd_ShadeColor.rgb*_2nd_ShadeMap_var.rgb);
                // _2nd_ShadeColor * _2nd_ShadeMap_var * _LightColor0
                float3 Set_2nd_ShadeColor = lerp( node_8559, (node_8559*Set_LightColor), _Is_LightColor_2nd_Shade );
                ...
                float4 _Set_1st_ShadePosition_var = tex2D(_Set_1st_ShadePosition,TRANSFORM_TEX(node_1237, _Set_1st_ShadePosition));
                float Set_FinalShadowSample = saturate((node_8323 + ( (lerp( node_4315, (node_4315*saturate(((attenuation*node_3494)+node_3494+_Tweak_SystemShadowsLevel))), _Set_SystemShadowsToBase ) - node_583) * ((1.0 - _Set_1st_ShadePosition_var.rgb).r - node_8323) ) / (_BaseColor_Step - node_583)));
                float3 node_1702 = lerp(Set_BaseColor,lerp(Set_1st_ShadeColor,Set_2nd_ShadeColor,saturate((node_9309 + ( (node_4315 - node_2294) * ((1.0 - _Set_2nd_ShadePosition_var.rgb).r - node_9309) ) / (_ShadeColor_Step - node_2294)))),Set_FinalShadowSample) * attenuation; // Final Color
                float2 node_4506 = Set_UV0;
                float4 _Set_HighColorPosition_var = tex2D(_Set_HighColorPosition,TRANSFORM_TEX(node_4506, _Set_HighColorPosition));
                float node_1331 = 0.5*dot(halfDirection,lerp( i.normalDir, normalDirection, _Is_NormalMapToHighColor ))+0.5; //  Specular
                float node_6262 = ((1.0 - _Set_HighColorPosition_var.rgb).r+lerp( (1.0 - step(node_1331,(1.0 - _HighColor_Power))), pow(node_1331,exp2(lerp(11,1,_HighColor_Power))), _Is_SpecularToHighColor ));
                ...
                float3 node_4172 = Set_MatCap;
                float3 finalColor = (saturate(lerp( _RimLight_var, lerp( (_RimLight_var*node_4172), (_RimLight_var+node_4172), _Is_BlendAddToMatCap ), _MatCap ))+(DecodeLightProbe( normalDirection )*_GI_Intensity));
                fixed4 finalRGBA = fixed4(DepressLighting(finalColor, node_9970 * attenuation, 0.7),0);
