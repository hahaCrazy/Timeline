...
//ToonCoreStart
       ...
       /*     float3 LightColor() {
                ...
                float colorNormal = _LightColor0.w;
                return colorNormal > 1 ? color * (1 / colorNormal)* (1 + (colorNormal - 1)*0.3) : color;
            }*/

            float3 LightColor() {
                float3 color = _LightColor0.rgb;
                float colorNormal = _LightColor0.w;
                return colorNormal > 1 ? color * (1 / colorNormal) : (colorNormal < 0.4 ? (color * (0.4 / colorNormal)) : color);
            }
